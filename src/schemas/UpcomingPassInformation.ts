import { z } from "zod";
import { PassFieldContent } from "./PassFieldContent.js";
import { Semantics } from "./Semantics.js";
import { URL_REGEX } from "./regexps.js";

/**
 * @iOSVersion 26
 * @see https://developer.apple.com/documentation/walletpasses/upcomingpassinformationentrytype/imageurlentry-data.dictionary
 */
type ImageURLEntry = z.infer<typeof ImageURLEntry>;

const ImageURLEntry = z.object({
	/** The SHA256 hash of the image. */
	SHA256: z.string(),

	/** The URL that points to the image asset to be downloaded. This must be an https link. */
	URL: z.string().check(z.regex(URL_REGEX)),

	/** The scale of the image. If unspecified, defaults to 1. */
	scale: z.number().default(1).optional(),

	/** Size of the image asset in bytes. The maximum allowed size is 2 megabytes. */
	size: z
		.number()
		.max(2 * 1024 * 1024)
		.optional(), // 2 megabytes max
});

/**
 * @iOSVersion 26
 * @see https://developer.apple.com/documentation/walletpasses/upcomingpassinformationentrytype/image-data.dictionary
 */
type Image = z.infer<typeof Image>;

const Image = z.discriminatedUnion("reuseExisting", [
	z.object({
		URLs: z.undefined().or(z.never()),
		/**
		 * Indicates whether to use the local equivalent image instead of the image specified by URLs.
		 */
		reuseExisting: z.literal(true),
	}),
	z.object({
		/**
		 * A list of URLs used to retrieve an image.
		 * The upcoming pass information entry uses the item that best matches the device's scale.
		 */
		URLs: z.array(ImageURLEntry),

		/**
		 * Indicates whether to use the local equivalent image instead of the image specified by URLs.
		 */
		reuseExisting: z.literal(false).or(z.never()).optional(),
	}),
]);

/**
 * @iOSVersion 26
 * @see https://developer.apple.com/documentation/walletpasses/upcomingpassinformationentry/images-data.dictionary
 */
type Images = z.infer<typeof Images>;

const Images = z.object({
	/**
	 * The name of the image file used for the header image on the details screen.
	 * This can be a remote asset.
	 */
	headerImage: Image.optional(),

	/**
	 * The name of the image file used for the venue map in the event guide for each upcoming pass information entry.
	 * This can be a remote asset and is available for event entries.
	 */
	venueMap: Image.optional(),
});

/**
 * @iOSVersion 26
 * @see https://developer.apple.com/documentation/walletpasses/upcomingpassinformationentry/urls-data.dictionary
 */
type URLs = z.infer<typeof URLs>;

const URLs = z.object({
	/**
	 * A URL that links to your or the venue's accessibility content.
	 */
	accessibilityURL: z.string().check(z.regex(URL_REGEX)).optional(),

	/**
	 * A URL that links to experiences that you can add on to your ticket
	 * or that allows you to access your existing prepurchased or preloaded
	 * add-on experiences, including any necessary QR or barcode links to
	 * access the experience. For example, loaded value or upgrades for
	 * an experience.
	 */
	addOnURL: z.string().check(z.regex(URL_REGEX)).optional(),

	/**
	 * A URL that links out to the bag policy of the venue.
	 */
	bagPolicyURL: z.string().check(z.regex(URL_REGEX)).optional(),

	/**
	 * Joi's email schema validates email TLDs and only allows TLDs that are registered in the IANA Registry.
	 * This also requires NodeJS runtime to work (even though can be enabled client side, but discouraged)
	 * Reference - https://github.com/hapijs/joi/issues/2390
	 *
	 * @TODO check if this is still an issue in zod
	 */

	/**
	 * The preferred email address to contact the venue, event, or issuer.
	 */
	contactVenueEmail: z.email().optional(),

	/**
	 * The preferred phone number to contact the venue, event, or issuer.
	 */
	contactVenuePhoneNumber: z.string().optional(),

	/**
	 * A URL that links the user to the website of the venue, event, or issuer.
	 */
	contactVenueWebsite: z.string().check(z.regex(URL_REGEX)).optional(),

	/**
	 * A URL that links to content you have about getting to the venue.
	 */
	directionsInformationURL: z.string().check(z.regex(URL_REGEX)).optional(),

	/**
	 * A URL that links to order merchandise for the specific event.
	 * This can be a ship-to-home ecommerce site, a pre-order to pickup at the
	 * venue, or other appropriate merchandise flow.
	 *
	 * This link can also be updated throughout the user's journey to provide
	 * more accurately tailored links at certain times.
	 *
	 * For example, before versus after a user enters an event.
	 * This can be done through a pass update. For more information on updating a
	 * pass, see Distributing and updating a pass.
	 */
	merchandiseURL: z.string().check(z.regex(URL_REGEX)).optional(),

	/**
	 * A URL that links out to the food-ordering page for the venue.
	 * This can be in-seat food delivery, pre-order for pickup at a vendor,
	 * or other appropriate food-ordering service.
	 */
	orderFoodURL: z.string().check(z.regex(URL_REGEX)).optional(),

	/**
	 * A URL that links to any information you have about parking.
	 */
	parkingInformationURL: z.string().check(z.regex(URL_REGEX)).optional(),

	/**
	 * A URL that links to your experience to buy or access prepaid parking
	 * or general parking information.
	 */
	purchaseParkingURL: z.string().check(z.regex(URL_REGEX)).optional(),

	/**
	 * A URL that launches the user into the issuer's flow for selling their
	 * current ticket. Provide as deep a link as possible into the sale flow.
	 */
	sellURL: z.string().check(z.regex(URL_REGEX)).optional(),

	/**
	 * A URL that launches the user into the issuer's flow for transferring
	 * the current ticket. Provide as deep a link as possible into the transfer flow.
	 */
	transferURL: z.string().check(z.regex(URL_REGEX)).optional(),

	/**
	 * A URL that links to documentation you have about public or private
	 * transit to the venue.
	 */
	transitInformationURL: z.string().check(z.regex(URL_REGEX)).optional(),
});

/**
 * @iOSVersion 26
 * @see https://developer.apple.com/documentation/walletpasses/upcomingpassinformationentry/dateinformation-data.dictionary
 */
type DateInformation = z.infer<typeof DateInformation>;

const DateInformation = z.object({
	/**
	 * A string containing an ISO 8601 date and time.
	 * The date and time when the event is scheduled.
	 */
	date: z.iso
		.datetime({
			offset: true,
		})
		.optional(),

	/**
	 * A Boolean value that controls whether the time appears on the pass.
	 * When true, the pass displays only the date, not the time.
	 */
	ignoreTimeComponents: z.boolean().optional(),

	/**
	 * A Boolean value that indicates whether the event lasts all day.
	 * When true, the system ignores the time portion of the date.
	 */
	isAllDay: z.boolean().optional(),

	/**
	 * A Boolean value that indicates whether the event time is unannounced.
	 * When true, the pass displays "Time TBA" instead of the actual time.
	 */
	isUnannounced: z.boolean().optional(),

	/**
	 * A Boolean value that indicates whether the event time is undetermined.
	 * When true, the pass may display the date differently to indicate uncertainty.
	 */
	isUndetermined: z.boolean().optional(),

	/**
	 * The time zone for the event.
	 * Use IANA time zone database names (e.g., "America/New_York").
	 */
	timeZone: z.string().optional(),
});

/**
 * @iOSVersion 26
 * @see https://developer.apple.com/documentation/walletpasses/upcomingpassinformationentry
 */
export type UpcomingPassInformationEntry = z.infer<
	typeof UpcomingPassInformationEntry
>;

export const UpcomingPassInformationEntry = z.object({
	/**
	 * The type of upcoming pass information entry.
	 * Value: event
	 */
	type: z.literal("event").default("event"),

	/**
	 * A string that uniquely identifies the upcoming pass information entry.
	 * The identifier needs to be unique for each upcoming information entry.
	 */
	identifier: z.string(),

	/** The name of the upcoming pass information entry. */
	name: z.string(),

	/**
	 * Indicates whether the upcoming pass information entry is currently active.
	 * The default value is false.
	 */
	isActive: z.boolean().optional().default(false),

	/** A collection of URLs used to populate UI elements in the details view. */
	URLs: URLs.optional(),

	/** The fields of information displayed on the Additional Info section below a pass. */
	additionalInfoFields: z.array(PassFieldContent).optional(),

	/**
	 * An array of App Store identifiers for apps associated with the upcoming pass information entry.
	 * The associated app on a device is the first item in the array that's compatible with that device.
	 * This key works only for upcoming pass information entries for an event. A link to launch the app
	 * is in the event guide of the entry details view. If the app isn't installed, the link opens to the App Store.
	 */
	auxiliaryStoreIdentifiers: z.array(z.number()).optional(),

	/** The fields of information displayed on the details view of the upcoming pass information entry. */
	backFields: z.array(PassFieldContent).optional(),

	/**
	 * Information about the start and end time of the upcoming pass information entry.
	 * If omitted, the entry is labeled as TBD.
	 */
	dateInformation: DateInformation.optional(),

	/** A collection of image names used to populate images in the details view. */
	images: Images.optional(),

	/** The semantic, machine-readable metadata about the upcoming pass information entry. */
	semantics: Semantics.extend(
		z.object({
			venuePlaceID: z.string().optional(),
		}),
	).optional(),
});
