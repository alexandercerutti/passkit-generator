...
const { default: FieldsArray } = require("../lib/FieldsArray");
const { filesSymbol, freezeSymbol } = require("../lib/Bundle");
...
/**
 * @typedef {import("../lib/schemas").PassFields} PassFields
 * @typedef {import("../lib/schemas").PassProps} PassProps
 */
...
describe("PKPass", () => {
	...
	describe("setBeacons", () => {
		it("should reset instance.props['beacons'] if 'null' is passed as value", () => {
			...
			if (!pass.props["beacons"]) {
				throw new Error(
					`Missing beacons object inside props. Something is definitely wrong here.`,
				);
			}
	...
	describe("setLocations", () => {
		it("should reset instance.props['locations'] if 'null' is passed as value", () => {
			...
			if (!pass.props["locations"]) {
				throw new Error(
					`Missing locations object inside props. Something is definitely wrong here.`,
				);
			}
	...
	describe("setExpirationDate", () => {
		...
		it("should throw if an invalid date is received", () => {
			...
			expect(() => pass.setExpirationDate(5)).toThrowError(
				new TypeError("Cannot set expirationDate. Invalid date 5"),
			);
	...
	describe("setRelevantDate", () => {
		...
		it("should throw if an invalid date is received", () => {
			...
			expect(() => pass.setRelevantDate(5)).toThrowError(
				new TypeError("Cannot set relevantDate. Invalid date 5"),
			);
	...
	describe("setBarcodes", () => {
		...
		it("should add 'messageEncoding' if missing in valid Schema.Barcode parameters", () => {
			...
			if (!pass.props["barcodes"]) {
				throw new Error(
					"Missing barcodes object inside props. Something is definitely wrong here.",
				);
			}
		...
		it("should ignore objects and values that not comply with Schema.Barcodes", () => {
			/**
			 * @type {Parameters<typeof pass["setBarcodes"]>}
			 */
	...
	describe("transitType", () => {
		it("should accept a new value only if the pass is a boarding pass", () => {
			...
			expect(
				...
				new TypeError(Messages.TRANSIT_TYPE.UNEXPECTED_PASS_TYPE),
			);
...
"ErrorString_1= "An unknown error occurred."
	...
	describe("[importMetadataSymbol]", () => {
		...
		it("should push fields to their own fields if a type is found", () => {
			/**
			 * @type {PassFields["headerFields"][0]}
			 */
	...
	describe("[closePassSymbol]", () => {
		beforeEach(() => {
			...
			pass.addBuffer(
				"pass.json",
				Buffer.from(JSON.stringify(partialPassJson)),
			);
		});
		...
		it("should throw if no pass type have specified", () => {
			...
			expect(() => pass[closePassSymbol](true)).toThrowError(
				new TypeError(Messages.CLOSE.MISSING_TYPE),
			);
		});
		...
		it("should throw if a boarding pass is exported without a transitType", () => {
			...
			expect(() => pass[closePassSymbol](true)).toThrowError(
				new TypeError(Messages.CLOSE.MISSING_TRANSIT_TYPE),
			);
		});
	});
	...
	describe("[static] pack", () => {
		...
		it("should should throw error if not all the files passed are PKPasses", () => {
			...
			).toThrowError(new Error(Messages.PACK.INVALID));
		});
